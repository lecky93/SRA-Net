...
class WeaklyNet6(BaseModel):
    ...
    def loss(self,cls_out, feats_cat, feats_residual, decoded_mask, datasample):
        labels = []
        cls_labels = []
    ...
    def forward(self, inputs, datasample=None, mode='loss'):
        ...
        map_hat = self.residual_conv(encoded_feats[-1])

        residual = abs(map_hat - feats_residual)
        ...
        if mode == 'loss':
            loss = self.loss(cls_out, map_hat, feats_residual, decoded_mask, datasample)
            return loss
        elif mode == 'predict':
