...
class WeaklyNet3(BaseModel):
    # use the cat feats for da_net, add cls_head, cam, feats_cat * cam
    def __init__(self):
        super().__init__()
    ...
    def forward(self, inputs, datasample, mode):
        ...
        map = resize(cam, encoded_feats[-1].shape[2:])
        feats_cat = feats_cat * map

        decoded_mask, pam_mask, cam_mask = self.decode_head([feats_cat])
